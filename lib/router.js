...
Router.prototype.match = function(pattern, shorthand, _options) {
  ...
  for (var i = 0, len = methods.length; i < len; i++) {
    ...
    if (typeof shorthand === 'function' || Array.isArray(shorthand)) {
      ...
      this._route(method, path, controller, action, helper, options);
    }
  }
};
...
Router.prototype.resource = function(name, options, fn) {
  ...
  if (options.as) {
    path = ns.qpath(options.as)
  }

  var self = this;
  actions.forEach(function(action) {
    switch (action) {
      case 'new':     self._route('get' , path + '/new.:format?' , controller, 'new'     , utils.functionize('new', helper), options);
        break;
      case 'create':  self._route('post', path                   , controller, 'create'  , null, options);
        break;
      case 'show':    self._route('get' , path + '.:format?'     , controller, 'show'    , helper, options);
        break;
      case 'edit':    self._route('get' , path + '/edit.:format?', controller, 'edit'    , utils.functionize('edit', helper), options);
        break;
      case 'update':  self._route('put' , path                   , controller, 'update'  , null, options);
        break;
      case 'destroy': self._route('del' , path                   , controller, 'destroy' , null, options);
        break;
    }
  });
  ...
  this.namespace(p, { module: options.namespace ? p : null, method: p }, function() {
    fn && fn.call(this);
  });
};
...
Router.prototype.resources = function(name, options, fn) {
  ...
  actions.forEach(function(action) {
    switch (action) {
      case 'index':   self._route('get' , path + '.:format?'                   , controller, 'index'   , collectionHelper, options);
        break;
      case 'new':     self._route('get' , path + '/new.:format?'               , controller, 'new'     , utils.functionize('new', helper), options);
        break;
      case 'create':  self._route('post', path                                 , controller, 'create'  , null, options );
        break;
      case 'show':    self._route('get' , path + '/' + param + '.:format?'     , controller, 'show'    , helper, options);
        break;
      case 'edit':    self._route('get' , path + '/' + param + '/edit.:format?', controller, 'edit'    , utils.functionize('edit', helper), options);
        break;
      case 'update':  self._route('put' , path + '/' + param                   , controller, 'update'  , null, options );
        break;
      case 'destroy': self._route('del' , path + '/' + param                   , controller, 'destroy' , null, options );
        break;
    }
  });
  ...
  this.namespace(p + '/' + placeholder, { module: options.namespace ? p : null, method: s }, function() {
    fn && fn.call(this);
  });
};
 ...
 * @api private
 */
Router.prototype._route = function(method, pattern, controller, action, helper, options) {
  // Define a route handler that dispatches to a controller action.
  // Send pattern and options to the handler.
  this.define(method, pattern, this._handler(controller, action, pattern, options));
